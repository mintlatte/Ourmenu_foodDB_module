...
public class IntentActivity extends AppCompatActivity {
    ...
    Handler con_handler;
    Handler cse_handler;
    Handler glide_handler;


    ArrayList<String> urlsForGlide = new ArrayList<String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtView = (TextView) findViewById(R.id.food_name_textView);
        ...
        Thread Glide_Thread = new Thread() {
            @Override
            public void run() {
                // Bundle foodInfoBundle = new Bundle();
                ImageUsingGlide imageUsingGlide = new ImageUsingGlide();
                ArrayList<String> urlsForGlide = imageUsingGlide.getFoodUrls(food_txt);
                boolean noData = true;

                if(urlsForGlide.size() == 0) {
                    noData = true;
                    food_ingre.setText("No Food Data");
                }
                else {
                    noData = false;
                    setImageUsingGlide(urlsForGlide);
                    food_ingre.setText("Searched by Google Custom Search Engine");
                    //foodInfoBundle.putStringArrayList("urlsForGlide", urlsForGlide);
                }
                /*
                foodInfoBundle.putBoolean("noData", noData);
                Message foodInfoMsg = glide_handler.obtainMessage();
                foodInfoMsg.setData(foodInfoBundle);
                cse_handler.sendMessage(foodInfoMsg);
                */
            };
        ...
        glide_handler = new Handler() {
            public void handleMessage(Message msg) {
                ...
                if(foodInfoBundle.getBoolean("noData") == true) {
                    ...
                    try {
                        ArrayList<String> urlsForGlide = foodInfoBundle.getStringArrayList("urlsForGlide");
                        Glide
                                .with(this)
                                .load(urlsForGlide.get(0))
                                .into(food_img_view);
    ...
    public void setImageUsingGlide(ArrayList<String> urlsForGlide) {
        Glide
                .with(this)
                .load(urlsForGlide.get(0))
                .into(food_img_view);
